...
$pag = 'GestionarRolesUsuarios2ET1';
if(!ConPermisos($pag)){
...
<html>
	...
	<?php
	$log=$_POST["user"];
	include '../../controllers/ConsultaUsuarioRol.php';
	?>


	<body link="#FFFFFF">
		<div style="width:800px; height:950px; overflow:hidden; margin: 0 auto; position:relative; margin-top:0px;">
			<div id="header">
				<span><h1><?php echo $Idioma['Gestionar Rol/Usuarios'] ?></h1></span>
			</div>
			<div id="section">
				<form action="gru3.php" method="post" class="elegant-aero">
				<label>
				<span><?php echo $Idioma['Roles asignados a'] ?>:</span>
				</label>
					<input id="log" type="text" name="log" readonly="readonly" value="<?php echo $log ?>" placeholder="<?php echo $log ?>" />
					<div div class="inline">
							<label>
								<span><?php echo $Idioma['Funcionalidades rol'] ?></span>
									<select multiple id="sel1" name="roles[]">
										<?php
									...
									<label style="display: inline-block; width:338px;">
										<span><?php echo $Idioma['Funcionalidades'] ?></span>
										<select id="sel2" name="rol" multiple style="width:180px;">
											<?php
												tablaF($log);
											?>
										</select>
					...
					<a href="gru.php" class="menuButton mainMenuButton"><?php echo $Idioma['boton_cancelar'] ?></a>
